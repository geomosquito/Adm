...
function exportar(){
    window.location.href = "../api/exportarEspecies.php";
}

function apagarEspecie(id, especie){
    $('.preloader').fadeIn();
    ...
    xhr.open("POST", `../api/apagarEspecie.php`);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.send(`id=${id}&especie=${especie}`);
}

function apagarPergunta(id, especie){
    Swal.fire({
        title: 'Exclusão!',
        text: `Tem certeza que quer excluir a espécie [${especie}]?`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
        confirmButtonText: 'Sim, Apagar essa espécie!',
        cancelButtonText: 'Cancelar',
      }).then((result) => {
        if (result.isConfirmed) {
            apagarEspecie(id, especie);
        }
      });
}
...
function gravarEspecie() {
    ...
    //Define o parâmetros para fazer um AJAX
    xhr.open("POST", `../api/gravarEspecie.php`);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.send(`id=${id}&especie=${especie}`);
}